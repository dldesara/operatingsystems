...
func (h *Host) manage() {
	for {
		if len(h.requestChannel) == NUM_EATING_PHILOSOPHER {
			...
			for ID, eating := range h.eatingPhilosophers {
				if eating {
					currentlyEating = append(currentlyEating, ID)
				}
			}
...
func main() {
	...
	//Declaring a waitgroup to keep track of threads
	...
	// quit channel signals host to stop managing becuase we're ending the program
